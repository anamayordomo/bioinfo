...
Esto es porque son archivos comprimidos, desde el terminal uso zless DRR +tab 1 para seleccionar el archivo.
:::
...
- ¿Cómo podrías guardar los mensajes de `wget` en un archivo? si en lugar de decir /dev/null me los mostrará en pantalla o ponerle -o y nombre del archivo 
- ¿De cuántas secuencias diferentes se compone este genoma de referencia? lo podemos ver en el terminal de bash donde está grep donde puedo interrogar un archivo y pedirle que me muestre las líneas con la expresión regular que le voy a dar: "^>" nombredelarchivo. Y te mostrará las lineas donde este > al principio ^.
:::
...
2. Revisa el informe generado por `fastp` en el archivo `fastp.html` y redacta un
   ...
   Las primeras posiciones no tienen una base clara. 
