...
Veamos primero de qué opciones disponemos:lo que hacemos es desacargar el genoma y lo enviamos por una tuberia (barra longitudinal), y la barra doblada es que bajamos de linea.Dtasets es para descargar cosas del MCI, con el comando summary. Si escribo summary genome taxone tripanosoma me selecciona todos los datos de taxones de tripanosoma, pero muy desordenado (el formato as json lines permite que pueda usarlo dataformat). Con dataforma lo que hago es poner la información en tabla, tsv es el formato en el que lo quiero, y luego le digo lo que quiero saber.

```{bash}
if [ ! -e  trypanosoma_annotated.txt ]; then
   ~/Descargas/datasets summary genome taxon Trypanosoma --annotated --as-json-lines | \
   ~/Descargas/dataformat tsv genome --fields \
...
## Desafío
1. ¿Qué diferencia hay entre las líneas 7 y 8 de la tabla anterior? Cmabia el GCF por GCA,  GCF representa un ensamblaje de referencia curado por NCBI, mientras que GCA representa ensamblajes primarios enviados directamente por investigadores, son secuencias originales, no necesariamente curadas o anatodas. Se utiliza una base de datos propia y una base de datos compartida, por eso cambian de nombre. 
2. Busca una función de R que te permita identificar las líneas repetidas en un
   marco de datos. No encontramos ninguna linea repetida porque nos aparece todo FALSE.
:::
...
if [ ! -e ncbi_data/data/GCA_022059095.1/genomic.gff ]; then
   if [ ! -e ncbi_dataset.zip ]; then
      ~/Descargas/datasets download genome accession GCA_022059095.1 --include genome,gff3
   fi
