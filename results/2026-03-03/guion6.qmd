...
taxonomy <- fromJSON(paste0(API, 'taxonomy'))[[1]]
```
:::
...
```{r}
library('kableExtra')
#Kable es para generar tablas elegantes
pfam02 <- fromJSON(pfam01[['next']])
#creo pfam02 con las 20 siguientes partiendo de pfam01
...
library('kableExtra')
#Kable es para generar tablas elegantes
pfam03 <- fromJSON(pfam01[['next']])
#creo pfam03 con las 20 siguientes partiendo de pfam02
...
```
```{r}
sequence <- p01923[["metadata"]][["sequence"]]
sequence
```
:::
...
Si restamos la URL de proteína total con aquella URL que solo me da las proteínas que tienen un entry. 
...
https://www.ebi.ac.uk/interpro/api/protein/taxonomy --> 2678841
https://www.ebi.ac.uk/interpro/api/taxonomy -->  1291419
Por lo que hay 1387422 taxones sin proteína asignada, es posible que sea porque entre las proteínas no revisadas han dejado de añadir aquellos que eran muy parecidos.

https://www.ebi.ac.uk/interpro/api/protein --> 202150435
https://www.ebi.ac.uk/interpro/api/protein/structure --> 228088
Por lo que hay 0,11% de proteínas con estructura terciaria
...
```{r}
taxon7742 <- fromJSON(paste0(API, 'entry/pfam/taxonomy/uniprot/7742'))
names(taxon7742)
taxon7742[['count']]
names(taxon7742[['results']])

# La función "flaten()" corrige el anidamiento de marcos de datos,
kable(flatten(taxon7742[['results']][['metadata']])) %>%
   kable_styling()
```
...
```{r}
taxon8493 <- fromJSON(paste0(API, 'protein/taxonomy/uniprot/8493/entry/pfam/PF01530'))
```
:::
