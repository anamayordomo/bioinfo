...
En el primer ejecutable hay un error porque la URL tiene un mal formato o porque no está la URL, pero nosotros sí tenemos la URL, por lo que el error tiene que estar en formato. La URL se puede abrir con el navegador pero es capaz de diferenciar espacios y comillas que R no es capaz, por eso URLencode permite diferenciar los signos y que no se detenga en un espacio o comillas. 

El segundo ejeutacble ha funcionado. La diferencia es que el segundo tiene un URLencode que permite que el formato de URL sea leido. 

El tercer ejecutable me da los valores que le solicito, pero no llega a descargar el z2.txt. Aunque no hay error tampoco. 

*Si quiero ejecutar solo una linea puedo situarme en ella y control + enter 
...
fields <- 'fields=tax_id,scientific_name,last_updated,base_count,base_count,first_public,description&'
URL <- paste0(portal, result, query, fields, limit)
curl_download(URLencode(URL), destfile = 'z3.txt')
```

1. ¿Cuántas secuencias hay con el término "alcohol dehydrogenase" en su descripción? Hay 1259 (dim(adh))
2. ¿Qué longitudes tienen? La media de longitudes es 1252 summary(adh$base_count)
3. ¿De cuántas especies diferentes son estas secuencias? 308 
unique(adh$scientific_name): para ver la columna solo
lenth(adh$scientific_name)
4. ¿Cuándo se publicaron las más recientes? El 2022-04-03
max(adh$first_public) para ver el valor máximo
tail(sort(adh$first_public)) para ver la cola del vector ordenado

Para poder ver los datos hago una tabla: 
...
```{r}
curl_download(URL, destfile = 'Adh.embl')
```
:::
